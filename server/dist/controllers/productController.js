...
const getAllProductsController = async (req, res) => {
    try {
        ...
        const productsWithDetails = await Promise.all(products.map(async (product) => {
            const getCateName = await (0, Product_1.getCategoryNameById)(product.categoryId);
            const getBrandName = await (0, Product_1.getBrandNameById)(product.brandId);
            const variants = productsVariants
                .filter((variant) => variant.productId === product.id)
                .map((variant) => ({ name: variant.variant, price: variant.price }));
            const numberOfOrders = await (0, Order_1.getOrderByProductId)(product.id);
            const rating = await (0, Review_1.getReviewsByProductID)(product.id);
...
const createProductController = async (req, res) => {
    try {
        ...
        const checkifexist = await (0, Product_1.findProductByTitle)(title);
        if (checkifexist) {
        ...
        if (variants.length > 0) {
            const newVariants = await Promise.all(variants.map(async (variant) => {
                const existingVariant = await (0, Product_1.findVariantByNameAndProductId)(variant.variant, newProduct.id);
                if (existingVariant) {
                    variantIds.push(existingVariant.id);
                    return existingVariant;
                }
                const newVariant = await (0, Product_1.createVariant)({
        ...
        if (colors.length > 0) {
            const newColors = await Promise.all(colors.map(async (color) => {
                const existingColor = await (0, Product_1.findColorByNameAndProductId)(color.name, newProduct.id);
                if (existingColor) {
                    colorIds.push(existingColor.id);
                    return existingColor;
                }
                const newColor = await (0, Product_1.createColor)({
                    productId: newProduct.id,
                    color: color.name,
                });
                if (newColor) {
...
const getProductController = async (req, res) => {
    try {
        const { id } = req.params;
        const getProduct = await (0, Product_1.findProductBySlug)(id);
        if (!getProduct) {
        ...
        const getCateName = await (0, Product_1.getCategoryNameById)(getProduct.categoryId);
        const getBrandName = await (0, Product_1.getBrandNameById)(getProduct.brandId);
        const variants = productsVariants
...
const getProductByID = async (req, res) => {
    try {
        const { id } = req.params;
        const getProduct = await (0, Product_1.findProductById)(id);
        if (!getProduct) {
            return res.status(404).send("Product not found");
        }
        return res.json({
...
const editProduct = async (req, res) => {
    try {
        ...
        const checkifexist = await (0, Product_1.findProductById)(id);
        if (!checkifexist) {
            return res.status(404).send("Product not found");
        }
        const findCategory = await (0, Product_1.getCategoryNameById)(categoryId);
        if (!findCategory) {
            return res.status(404).send("Category not found");
        }
        const findBrand = await (0, Product_1.getBrandNameById)(brandId);
        if (!findBrand) {
            return res.status(404).send("Brand not found");
        }
        const updateProductdetails = await (0, Product_1.updateProduct)({
            id,
            title,
            price,
            discounted,
        ...
        // Update variants
        if (variants.length > 0) {
            const existingVariants = await (0, Product_1.getVariantbyProductId)(id);
            const variantNames = variants.map((variant) => variant.id);
            // Delete variants that are no longer present
            existingVariants.forEach(async (existingVariant) => {
                if (!variantNames.includes(existingVariant.id)) {
                    await (0, Product_1.deleteVariant)(existingVariant.id);
                }
            });
        ...
        // Update colors
        if (colors.length > 0) {
            const existingColors = await (0, Product_1.getColorbyProductId)(id);
            const colorNames = colors.map((color) => color.id);
            // Delete colors that are no longer present
            existingColors.forEach(async (existingColor) => {
                if (!colorNames.includes(existingColor.id)) {
                    await (0, Product_1.deleteColor)(existingColor.id);
                }
            });
...
const getProductsByCate = async (req, res) => {
    try {
        ...
        const getcateID = await (0, Category_1.One)(catename);
        if (!getcateID) {
            return res.status(404).send("Category not found");
        }
        const products = await (0, Product_1.getProductsByCategory)(getcateID.id);
        if (!products) {
            return res.status(500).send("Failed to fetch products");
        }
        const addRating = await Promise.all(products.map(async (product) => {
            const rating = await (0, Review_1.getReviewsByProductID)(product.id);
            const allRatings = rating
                .map((review) => review.rating || 0)
                .reduce((acc, curr) => acc + curr, 0);
...
const getFilterData = async (req, res) => {
    ...
    try {
        ...
        const getCateID = await (0, Category_1.One)(formatedCatename);
        if (!getCateID) {
            return res.status(404).send("Category not found");
        }
        const getdata = await (0, Product_1.getProductFilterData)({ cateID: getCateID.id });
        if (!getdata) {
            return res.status(500).send("Failed to fetch products");
        }
        const allData = {
...
const getProductsBySearch = async (req, res) => {
    ...
    try {
        const products = await (0, Product_1.getSearchedProducts)(query);
        if (!products) {
            return res.status(500).send("Failed to fetch products");
        }
        return res.json({
...
const deleteProduct = async (req, res) => {
    ...
    try {
        ...
        const deleteProduct = await (0, Product_1.Delete)(id);
        if (!deleteProduct) {
            return res.status(404).send("Product not found");
        }
        return res.json({
